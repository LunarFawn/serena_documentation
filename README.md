...
## Aggressive vs Open
Serena's switchyness code has teh option now of beingh aggressive or more open in its scoring. More aggressive scoring means that the awarding of points is constrained adn you will have more good designs score less tahn 0. This may not be a bad thing nad I think that aggresive is teh better option right now. I think that Serena is indicating that if a switches echo for teh 2nd state is too stong then it will not fold right. You need to have a balance and it seams that agressive give a range of ~ 3 to -3 for designs with a fold change over 100 and very low foild change may have a score of around 10 to 14 for example. In the code there is no an example analysis of the Eterna PNAS Round 101 designs you can follow to run your own analysis. 
